package com.arash.neshan.test2.component.util
...
import com.arash.neshan.test2.component.view.snackbar.SnackBarType
 ...
 * */
fun Throwable.getError(): com.arash.neshan.test2.data.model.error.GeneralError {

    this.printStackTrace()

    return when (this) {
        is UnknownHostException, is SocketException -> {
            com.arash.neshan.test2.data.model.error.NetworkError.instance()
        }
        is SocketTimeoutException -> {
            com.arash.neshan.test2.data.model.error.TimeoutError.instance()
        }
        is HttpException -> {
            // TODO: improve parsing server errors
            return com.arash.neshan.test2.data.model.error.ServerError(this.response()?.code() ?: 0)
        }
        else -> {
            com.arash.neshan.test2.data.model.error.UnknownError.instance()
        }
    }
}
 ...
 * */
fun showError(rootView: View, error: com.arash.neshan.test2.data.model.error.GeneralError) {
    when (error) {
        is com.arash.neshan.test2.data.model.error.NetworkError -> {
            SnackBar.make(rootView, R.string.network_connection_error, SnackBarType.ERROR).show()
        }
        is com.arash.neshan.test2.data.model.error.ServerError -> {
            SnackBar.make(rootView, R.string.unknown_server_error, SnackBarType.ERROR).show()
        }
        is com.arash.neshan.test2.data.model.error.SimpleError -> {
            SnackBar.make(rootView, error.errorMessage, SnackBarType.ERROR).show()
        }
        is com.arash.neshan.test2.data.model.error.UnknownError -> {
